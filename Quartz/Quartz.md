...
关系: Scheduler可以注册多个JobDetail和Trigger, Trigger需要与JobDetail绑定, JobDetail需要与Job绑定
...
| /        | x/y表达⼀个等步⻓序列，x为起始值，y为增量步⻓值。如: 在分钟字段中使⽤0/15，则表示为0,15,30和45分 |
...
Quartz默认是将任务保存到内存, 但是它也支持将任务保存到数据库。
