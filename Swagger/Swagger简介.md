...
+ 并不是因为@ApiModel这个注解让实体显示在这里了，而是==只要出现在接口方法的返回值上的实体都会显示在这里==，而@ApiModel和@ApiModelProperty这两个注解只是为实体添加注释的。

    @ApiModel为类添加注释

    @ApiModelProperty为类属性添加注释

+ 如果实体类的属性为private, 则必须要有get方法, 否则页面无法显示实体类的属性。
...
Swagger是个优秀的工具，现在国内已经有很多的中小型互联网公司都在使用它，相较于传统的要先出Word接口文档再测试的方式，显然这样也更符合现在的快速迭代开发行情。当然了，提醒下大家在==正式环境要记得关闭Swagger==，一来出于安全考虑二来也可以节省运行时内存。
