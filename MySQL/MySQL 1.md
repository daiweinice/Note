...
**如何理解n个key和n+1个指针:** 比如最顶层G、M就是2个key, 那么他们有2+1=3个指针, 这三个指针分别指向小于G的区域、介于G和M的区域、大于M的区域
...
MySql索引数据结构对经典的B+Tree进行了优化。在原B+Tree的基础上，增加一个指向相邻叶子节点的链表指针，就形成了带有顺序指针的B+Tree，提高跨区间访问的性能(即提高跨区间范围查找的性能, 比如查询9到13)。
...
- 利用最左前缀，N个列组合而成的组合索引，那么相当于是创建了N个索引，如果查询时where子句中使用了组成该索引的前几个字段，那么这条查询SQL可以利用组合索引来提升查询效率。
  ...
  从上面可以看出, 有三个索引中包含了name, 所以使用频率最高的where条件字段, 应该放在复合索引的最前面(左端)
  ```
...
修改视图的语法为：修改视图就是修改原表, 原始表的数据会被修改。但是一般不建议更新视图, 因为视图的存在是为了简化我们的查询操作的。
