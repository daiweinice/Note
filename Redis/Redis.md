...
乐观锁虽然提高了吞吐量, 但是当多个并发线程对同一key进行操作时, 只能有一个操作成功, 其他都会失败(最快的成功后版本发生改变, 其他并发操作因为版本不匹配则会被取消)
...
Redis通过命令 `watch key1 [key2]`来实现对key的监听, 监听的key在Multi命令之后如果有其他客户端命令对该key进行了操作,那么事务会被中断, 命令不会被执行.
...
由于乐观锁会导致并发拒绝大部分操作, 使用lua脚本解决库存堆积问题